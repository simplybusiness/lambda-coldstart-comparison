...
let readStats = co.wrap(function* () {
  let contents = yield fs.readFileAsync('all-traces.csv', 'utf-8');
  let rows = contents
    .split('\n')
...
let boxPlot = co.wrap(function*() {
  ...
  let byPhase = {
    setup: { y: [], x: [], type: "box", boxpoints: "all", name: "setup" },
    initialization: { y: [], x: [], type: "box", boxpoints: "all", name: "initialization" },
    execution: { y: [], x: [], type: "box", boxpoints: "all", name: "execution" },
  }

  rows.forEach(row => {
    if (Object.keys(byLang).includes(row.lang)) {
      // byLang[row.lang].y.push(row.total);
      // byLang[row.lang].x.push(`${row.memorySize}MB`);
      byPhase.setup.y.push(row.setup);
      byPhase.setup.x.push(`${row.memorySize}MB`);
      byPhase.initialization.y.push(row.initialization);
      byPhase.initialization.x.push(`${row.memorySize}MB`);
      byPhase.execution.y.push(row.execution);
      byPhase.execution.x.push(`${row.memorySize}MB`);
    }
  });

  // let data = _.values(byLang);
  let data = _.values(byPhase);
  let layout = {
    yaxis: {
